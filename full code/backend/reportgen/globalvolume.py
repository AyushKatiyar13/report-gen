...
#             i["vsplit"][str(k)]=(s1*float(year[str(k)]))/100
#             s1+=step
#         region(i['tocreg'],i["vsplit"],g)

# def region(toc,year,g):
#     for i in toc:
#         s1=float(i["vsplit"][0])
#         s2=float(i["vsplit"][1])
#         step=(float(s2)-float(s1))/(int(g[1])-int(g[0]))
#         i["vsplit"]={}
#         for k in range (g[0],g[1]+1):
#             i["vsplit"][str(k)]=(float(s1)*float(year[str(k)]))/100
#             s1+=step
#         tocl2reg(i["toc"],i["vsplit"],g)
#         tocl3reg(i["toc"],g)
#         tocl4reg(i["toc"],g)
...
#             j["vsplit"]={}
#             for k in range (g[0],g[1]+1):
#                 j["vsplit"][str(k)]=(s1*float(year1[str(k)]))/100
#                 s1+=step
...
#                     t["vsplit"]={}
#                     for k in range (g[0],(g[1]+1)):
#                         t["vsplit"][str(k)]=(s1*float(year[str(k)]))/100
#                         s1+=step
...
#                         r['vsplit']={}
#                         for k in range (g[0],(g[1]+1)):
#                             r['vsplit'][str(k)]=(s1*float(year[str(k)]))/100
#                             s1+=step   
...
def global1(toc,year,g):
    for i in toc:
        ...
        i["split"]={}
        for k in range (g[0],g[1]+1):
            i["vsplit"][str(k)]=float((s1*mpf(str(year[str(k)]).replace(",","")))/100)
            s1+=step
        region(i['tocreg'],i["vsplit"],g)
...
def tocl2reg(toc,year1,g):
    for i in toc:
        for j in (i["toc_component"]):
            ...
            j["vsplit"]={}
            for k in range (g[0],g[1]+1):
                j["vsplit"][str(k)]=float((s1*mpf(str(year1[str(k)]).replace(",","")))/100)
                s1+=step
...
def regcountvsplit(reg,toc,country):
    for l1 in toc:
        if l1["region"]==reg:
            for l2 in l1["tocreg"]:
...
def globalbyregion(toc,pdlist,market,count):
    ...
    for reg1 in toc: 
        pdlist["global "+market+" Market, by region"+"#"+str(count)]["data"][reg1["region"]]=reg1["vsplit"]
    count+=1
    return count
...
def regionbycountry(re,pdlist,market,toc,reg,count):
    ...
    for countr in reg[re]:
        pdlist[re+" "+market+" Market, By Country"+"#"+str(count)]["data"][countr]=regcountvsplit(re,toc,countr)
    count+=1
    return count
...
#     paragraph_format14 = doc.styles['FIG_no_NM'].paragraph_format
    ...
    doc.save(dir+"/files/"+"Global"+market+" Market Volume Report Forecast till "+str(g[1])+".docx")
    wb.save(dir+"/files/"+"Global"+market+" Market Volume Excel Report Forecast till "+str(g[1])+".xlsx")
    os.remove(pic)  

    return(("/files/"+"Global "+market+" Market Volume Report Forecast till "+str(g[1])+".docx","/files/"+"Global"+market+" Market Volume Excel Report Forecast till "+str(g[1])+".xlsx"))
    #     doc.add_page_break()
